...
export default function Main() {
  const { setTheme, theme, themeObj, setState, state } =
    React.useContext(MainContext);
  const { downloadAsCsv, downloadAsExcel, setFileData, setHeader } =
  ...
  return (
    <>
      <div
        className={`w-[100%] min-h-[100vh] flex justify-center items-center ${themeObj}`}
      >
        ...
        {state === 0 ? (
          ...
          >
            <div className="absolute right-0 top-2 flex items-center">
              <p
                onClick={newClicked}
                className="py-2 px-3 bg-blue-500 text-white mx-2 rounded-[10px] cursor-pointer"
              >
                New
              </p>
              <BsFiletypeCsv
